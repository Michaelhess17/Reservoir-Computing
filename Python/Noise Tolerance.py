...
def NARMA_Test(test_length=800, train_length=800, N=400, eta=0.35, gamma=0.05,
               tau=400, bits=np.inf, preload=False, mask=0.1, activate='mg', cv=False, beta=1.0, t=1,
               theta=0.2, noise_scale=0.001):
	"""
	Args:
...
def Noise_Test():
	...
	# Import u and m
	noises = np.logspace(-20, -1, 50)
	acts = ['mg', 'hayes']
	colors = ['r', 'b']
	for k, act in enumerate(acts):
		NRMSEs = []
		for noise in noises:
			NRMSEs.append(np.mean([NARMA_Test(noise_scale=noise, activate=act) for _ in range(5)]).item())
		plt.semilogx(noises, NRMSEs, c=colors[k], label=act)
	plt.xlabel('Scale of Noise')
	plt.ylabel('NRMSE')
	plt.legend()
	plt.savefig('Noise Tolerance.png', dpi=600)
