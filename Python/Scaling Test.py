...
def Noise_Test():
	...
	for scale in scales:
		for k, act in enumerate(acts):
			NRMSEs = []
			for noise in noises:
				NRMSEs.append(np.mean([NARMA_Test(noise_scale=noise, activate=act, scale=scale) for _ in range(3)]).item())
			plt.semilogx(noises, NRMSEs, c=colors[k], label=act)
		plt.xlabel('Scale of Noise')
		plt.ylabel('NRMSE')
		plt.legend()
		plt.savefig(f'Noise Tolerance with scale={scale}.png', dpi=600)
		plt.clf()


# Noise_Test()

NRMSE_scaled = NARMA_Test(scale=True)
NRMSE_regular  = NARMA_Test(scale=False)
print(f'Scaled NRMSE: {NRMSE_scaled}')
print(f'Regular NRMSE: {NRMSE_regular}')
