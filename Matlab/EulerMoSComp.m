...
%     ylabel("time(s) needed for Euler computation");
    ...
    %% 2000 points between 0.00001 -> 0.0001
...
%     ylabel("time(s) needed for Euler computation");
        ...
        %% Find the "characteristic" timescale of change in the dynamical system. 
            % How much t do we have to go to get some sort of significant
            % change?
            %
            % Generates nofSlice points and compares the accuracy of the
            % solutions generated by making the integration step slightly
            % smaller (by different amounts
            ...
            nofSlice = 50
            testRange = linspace(0.01,0.0001,nofSlice)
              for n = 2:nofSlice
                
                theta1 = testRange(n) - 0.0001;        % stable in the ball part of 3 magnitudes?
                [eulert_1, M_x_1] = MGEulerComp(tau, eta, p, gamma, iterations, theta1, initial);

                theta3 = testRange(n);
                [eulert_3, M_x_3] = MGEulerComp(tau, eta, p, gamma, iterations, theta3, initial);
                
                interpM_x_31 = interp1(eulert_1, M_x_1, eulert_3);      % Interpolates points of our solution at benchmark (more sparse) to more defined solution calculated with a smaller timestep
                interpM_x_32 = interp1(eulert_2, M_x_2, eulert_3);      
                diff(n-1,1) = sum(M_x_3 - interpM_x_31,2);              % compares the difference between the one at smaller time step and benchmark
                diff(n-1,2) = sum(M_x_3 - interpM_x_32,2);
...
%             subplot(3,1,2);
%             plot(eulert_1, M_x_1);
%             title("theta = 0.104 (one timestep smaller")
